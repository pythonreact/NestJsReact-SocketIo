...
const Home: React.FC = () => {
  ...
  const works = t('travel', { returnObjects: true, ns: ['home'] }) as string[];
  ...
  const dropText = katakana + latin + nums;
  const dropColor = '#0f0';
  const backgroundFillStyle = `rgba(0, 0, 0, ${dropOpacity})`; //backgroundcolor
  ...
  const drawDrop = useCallback(() => {
    ...
    if (ctx.current) {
      ctx.current.fillStyle = backgroundFillStyle; //background color
      ctx.current.fillRect(0, 0, canvasWidth.current, canvasHeight.current);
      ctx.current.fillStyle = dropColor ? dropColor : `rgb(${random()}, ${random()}, ${random()})`; // drop color
      ctx.current.font = `${fontSizeDrop.current}px ${fontFamilyDrop}`;
      const textDrop = dropText[count.current];
      for (let i = 0; i < drops.current.length; i++) {
        ctx.current.fillText(textDrop, drops.current[i].x, drops.current[i].y);
        ...
        worksMetrics.current.forEach((work) => {
          if (drops.current[i].x > work.x1 && drops.current[i].x < work.x2 && drops.current[i].y > work.y1 && drops.current[i].y < work.y2) {
